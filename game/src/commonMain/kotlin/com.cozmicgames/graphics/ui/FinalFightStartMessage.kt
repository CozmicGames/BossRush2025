...
class FinalFightStartMessage : GUIElement() {
    ...
    private inner class Stage0 : Stage {
        ...
        override fun update(delta: Duration): Stage {
            ...
            label.fontSize = 65.0f * (1.0f - Easing.QUAD_IN(factor))
    ...
    private inner class Stage2 : Stage {
        ...
        init {
            label.text = "Exterminate!"
        }

        override fun update(delta: Duration): Stage {
            val factor = (timer / 1.0.seconds).toFloat().clamp(0.0f, 1.0f)
            label.fontSize = 400.0f * Easing.QUAD_IN(factor)
            color.a = 0.85f * (1.0f - factor)

            timer += delta

            return if (timer > 1.0.seconds) Stage3() else this
        }
    }
    ...
    private val label = Label("Weapons set to kill mode!", 65.0f, color)
